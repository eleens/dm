...
func (rpv rsPoolValue) refreshed(conn *DmConnection) (bool, error) {
	...
	tss, err := conn.Access.Dm_build_151(interface{}(rpv.m_TbIds).([]uint32))
	if err != nil {
		return false, err
	}
	rpv.m_lastChkTime = time.Now().Nanosecond()
...
func (st *DmStatement) prepare() error {
	...
	st.execInfo, err = st.dmConn.Access.Dm_build_76(st, Dm_build_352)
	if err != nil {
		return err
	}
...
func (stmt *DmStatement) query(args []driver.Value) (*DmRows, error) {
	var err error
	stmt.inUse = true
	err = stmt.executeInner(args, Dm_build_353)
	if err != nil {
		return nil, err
	}
...
func (stmt *DmStatement) free() error {
	...
	err := stmt.dmConn.Access.Dm_build_63(int32(stmt.id))
	if err != nil {
		return err
	}
	delete(stmt.dmConn.stmtMap, stmt.id)
	...
	nextSwitch:
		if stmt.params[i].colType == CURSOR {
			if stmt.params[i].cursorStmt == nil {
				...
				err = stmt.params[i].cursorStmt.dmConn.Access.Dm_build_58(stmt.params[i].cursorStmt)
			}
...
func (stmt *DmStatement) executeInner(args []driver.Value, executeType int16) (err error) {
	...
	stmt.execInfo, err = stmt.dmConn.Access.Dm_build_108(stmt, bytes, false)
	if err != nil {
		return err
	}
			...
			if stmt.curRowBindIndicator[i]&BIND_OUT == BIND_OUT {
				if outParamData == nil {
					if arg, ok := args[i].(*driver.Rows); ok && stmt.params[i].colType == CURSOR {
						...
						if tmpExecInfo, err = stmt.dmConn.Access.Dm_build_118(stmt.params[i].cursorStmt, 1); err != nil {
							return err
						}
				...
				nextSwitch:
					...
					case *driver.Rows:
						if stmt.params[i].colType == CURSOR {
							var tmpExecInfo *execInfo
							tmpExecInfo, err = stmt.dmConn.Access.Dm_build_118(stmt.params[i].cursorStmt, 1)
							if err != nil {
								return err
							}
...
func (stmt *DmStatement) executeBatch(args []driver.Value) (err error) {
	...
	stmt.execInfo, err = stmt.dmConn.Access.Dm_build_97(stmt, bytes)
	return err
}
