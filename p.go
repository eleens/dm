...
const (
	...
	compressDef   = Dm_build_348
	compressIDDef = Dm_build_349
...
func (c *DmConnector) setAttributes(props *Properties) error {
	...
	c.bufPrefetch = props.GetInt(BufPrefetchKey, c.bufPrefetch, int(Dm_build_333), int(Dm_build_334))
...
func (c *DmConnector) parseOsAuthType(props *Properties) error {
	...
	if value != "" && !util.StringUtil.IsDigit(value) {
		if util.StringUtil.EqualsIgnoreCase(value, "ON") {
			...
			c.osAuthType = Dm_build_326
		}
	} else {
		c.osAuthType = byte(props.GetInt(OsAuthTypeKey, int(c.osAuthType), 0, 4))
	}
	if c.user == "" && c.osAuthType == Dm_build_326 {
		c.user = "SYSDBA"
	} else if c.osAuthType != Dm_build_326 && c.user != "" {
		return ECGO_OSAUTH_ERROR.throw()
	} else if c.osAuthType != Dm_build_326 {
...
func (c *DmConnector) connectSingle(ctx context.Context) (*DmConnection, error) {
	...
	dc.Access, err = dm_build_13(dc)
	if err != nil {
		return nil, err
	}

	if err = dc.Access.dm_build_54(); err != nil {
