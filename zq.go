...
const (
	READ_LEN = Dm_build_384
)

type iOffRowBinder interface {
	read(buf *Dm_build_1144)
	isReadOver() bool
	getObj() interface{}
}
...
func newOffRowBytesBinder(obj []byte, encoding string) *offRowBytesBinder {
	...
	binder.offRow = binder.buffer.Dm_build_1149() > Dm_build_381
	return binder
}

func (b *offRowBytesBinder) read(buf *Dm_build_1144) {
	if b.buffer.Dm_build_1149() > 0 {
		buf.Dm_build_1181(b.buffer)
	} else if !b.readOver {
		var obj = b.obj.([]byte)
		buf.Dm_build_1170(obj, 0, len(obj))
		b.readOver = true
	}
}
...
func (b *offRowBlobBinder) read(buf *Dm_build_1144) {
	if b.buffer.Dm_build_1149() > 0 {
		buf.Dm_build_1181(b.buffer)
	} else if !b.readOver {
		...
		buf.Dm_build_1170(bytes, 0, len(bytes))
	}
}
...
func (b *offRowClobBinder) read(buf *Dm_build_1144) {
	if b.buffer.Dm_build_1149() > 0 {
		buf.Dm_build_1181(b.buffer)
	} else if !b.readOver {
		...
		var bytes = Dm_build_868.Dm_build_1078(str, b.encoding, nil)
		b.position += readLen
		if b.position == int32(totalLen) {
			b.readOver = true
		}
...
func (b *offRowReaderBinder) read(buf *Dm_build_1144) {
	if b.buffer.Dm_build_1149() > 0 {
		buf.Dm_build_1181(b.buffer)
	} else if !b.readOver {
		var err error
		...
		buf.Dm_build_1170(bytes[0:readLen], 0, readLen)
	}
}

func (b *offRowReaderBinder) readAll() []byte {
	var byteArray = Dm_build_1148()
	b.read(byteArray)
	for !b.readOver {
		b.read(byteArray)
	}
	return byteArray.Dm_build_1191()
}
