...
func (conn *DmConnection) setTrxFinish(status int32) {
	switch status & Dm_build_389 {
	case Dm_build_386, Dm_build_387, Dm_build_388:
		conn.trxFinish = true
	default:
...
func (dc *DmConnection) executeInner(query string, execType int16) (interface{}, error) {
	...
	if execType == Dm_build_353 && dc.dmConnector.enRsCache {
	...
	if optParamList != nil && len(optParamList) > 0 {
		info, err = dc.Access.Dm_build_80(stmt, optParamList)
		if err != nil {
			stmt.nativeSql = query
			info, err = dc.Access.Dm_build_86(stmt, execType)
		}
	} else {
		info, err = dc.Access.Dm_build_86(stmt, execType)
	}
...
func (dc *DmConnection) reconnect() error {
	...
	for _, stmt := range dc.stmtMap {
		err = dc.Access.Dm_build_58(stmt)
		if err != nil {
			return err
		}
...
func (dc *DmConnection) close() error {
	...
	if dc.stmtPool != nil {
		for _, spi := range dc.stmtPool {
			err = dc.Access.Dm_build_63(spi.id)
			if err != nil {
				return err
			}
		}
...
func (dc *DmConnection) query(query string, args []driver.Value) (*DmRows, error) {
	...
	if args != nil && len(args) > 0 {
		...
		r1, err := dc.executeInner(query, Dm_build_353)
		if err != nil {
			return nil, err
		}
